...
fun DemoPanel(gatewayName: String?, gatewayBaseUrl: String?) {
    ...
    fun setGatewayState(infected: Boolean) {
        ...
        if (base == null) {
            message = "The gateway must be registered before its infection state can be changed."
            return
        }
        val current = gatewayState
        if (current != null && current.reachable && current.infected == infected) return

        busy = true
        message = if (infected) "Requesting infected state on gateway…" else "Requesting clean state on gateway…"
        Thread {
            val result = DemoClient.setGatewayInfected(base, infected)
            try { Thread.sleep(if (infected) 2000L else 700L) } catch (_: InterruptedException) { }
            val t = currentTarget()
            ...
            activity.runOnUiThread {
                if (identity != null) discoveredName = identity.nodeName
                gatewayState = gateway
                if (receiver != null) receiverState = receiver
                message = result + if (gateway.infected == infected) " — confirmed by gateway" else " — waiting for gateway state to propagate"
                busy = false
            }
        }.start()
    }

    // Keep the control and both displayed nodes synchronized with TaraSec itself.
    // No optimistic UI state is used: the radio selection changes only after
    // getTagData() on the gateway reports the changed state.
    ...
    Column(verticalArrangement = Arrangement.spacedBy(12.dp), modifier = Modifier.fillMaxWidth()) {
        ...
        Text(
            "WireGuard determines the route. Every displayed infection state is fetched from getTagData() on the node whose status is being shown.",
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(18.dp), modifier = Modifier.fillMaxWidth()) {
            Row {
                RadioButton(
                    ...
                    enabled = state?.reachable == true && !busy,
                    onClick = { setGatewayState(false) }
                )
                Text("Clean")
            }
            Row {
                RadioButton(
                    selected = state?.reachable == true && state.infected,
                    enabled = state?.reachable == true && !busy,
                    onClick = { setGatewayState(true) }
                )
                Text("Infected")
            }
        }
        Text(
            if (state == null) "Waiting for gateway…"
            else if (!state.reachable) "Gateway status unavailable"
            else "Reported by gateway: ${if (state.infected) "INFECTED" else "CLEAN"} · severity ${state.severity}",
            style = MaterialTheme.typography.bodySmall
        )
        ...
        gatewayState?.let { gateway ->
            TaraSectionCard(title = gatewayName ?: "Gateway", subtitle = "getTagData() on gateway") {
                ...
                if (gateway.reachable) {
                    TaraStatusRow("Unit state", if (gateway.infected) "🔴 INFECTED" else "🟢 CLEAN")
                    TaraStatusRow("Severity", gateway.severity.toString())
                    if (gateway.source.isNotBlank()) TaraStatusRow("Evidence", gateway.source)
                } else if (gateway.message.isNotBlank()) Text(gateway.message, style = MaterialTheme.typography.bodySmall)
            }
        }
        ...
        receiverState?.let { receiver ->
            TaraSectionCard(title = discoveredName, subtitle = "getTagData() on ${targetIp.trim()}") {
                ...
                if (receiver.reachable) {
                    TaraStatusRow("Reports this unit", if (receiver.infected) "🔴 INFECTED" else "🟢 CLEAN")
        ...
        Text(
            "The radio selection is not stored in the app. It follows the gateway's reported state, so changes made in Gatekeeper should appear here automatically as well.",
            style = MaterialTheme.typography.bodySmall
        )
    }
}
